...
int main()
{
    ...
    // 删除dset对象中的元素1.1
	set<double>::iterator er=dset.equal_range(1.1).first;
    dset.erase(er);


    // 删除dset对象中大于等于3.4且小于7.8的所有元素
    set<double>::iterator lw=dset.upper_bound(3.3);
    set<double>::iterator hw=dset.lower_bound(7.8);
    dset.erase(lw,hw);
