...
{	if(TreeIsFull(ptr))
	...
	Trnode *pnode;
	int x;
	if((pnode=SeekItem(&item,ptr).child)!=NULL)
	{	if(pnode->item.kinds==6)
		{	printf("This node is full\n");
			return 0;
		}
		for(x=0;x<=5;x++)
			if(!strcmp(pnode->item.petkind[x],item.petkind[0]))
			{	printf("It is already in the tree!\n");
				return 0;
			}
		for(x=0;x<=5;x++)
			if(pnode->item.petkind[x][0]=='\0')
			{	strcpy(pnode->item.petkind[x],item.petkind[0]);
				pnode->item.kinds++;
				break;
			}
		ptr->pets++;
...
{	Pair ppair=SeekItem(item,ptr);
	...
	int x;
	for(x=0;x<=5;x++)
		if(!strcmp(ppair.child->item.petkind[x],item->petkind[0]))
		{	ppair.child->item.petkind[x][0]='\0';
			ppair.child->item.kinds--;
			ptr->pets--;
			break;
		}
	if(x==6)
